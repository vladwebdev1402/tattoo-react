...
import { supportManagerData } from '@/data';
      ...
      <div className={styles.containerBodyOffice}>
        <div className={styles.bodyOfficeInfo}>
          ...
          <p className={`${styles.txt} ${styles.timing}`}>
            Понедельник – пятница: с 07:00 до 15:00 (МСК) <br /> Звонок бесплатный!
          </p>
          ...
          <p className={`${styles.txt} ${styles.workingMode}`}>
            Ваши заказы, звонки и обращения обрабатываются в рабочее время. Если Вы обратитесь к нам
            в иное время - мы свяжемся с Вами на следующий день. Мы обращаем внимание, в каком
            часовом поясе Вы находитесь, и не звоним в неудобное для Вас время.
          </p>
          <p className={`${styles.txt} ${styles.delivery}`}>
            Пункт выдачи в г. Благовещенске, Калинина 107-202, стоимость доставки 300 руб.,
            при оформлении заказа до 15.00 доставка в день заказа.
          </p>
          <p className={`${styles.txt} ${styles.nameDelivery}`}>ООО «Посылка»</p>
          <p className={`${styles.txt} ${styles.nameDelivery}`}>ОГРН: 1192036006645</p>
        </div>
